"""
Code for SVM - fitting.
"""
from methods import Regression
from helper_functions import DataWorkflow, CV
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
import pandas as pd
from  tqdm import tqdm
from sklearn.metrics import mean_squared_error
from pathlib import Path

# Function that performs a grid search
def SVM_GridCV(X,y,Xtest, ytest, eps_path, penalty_path,  folds, toi, fnum, reg):
    """
    Function that performs grid search for SVM linear regression
    """

    features = X.shape[1]
    for eps in eps_path:
        for penalty in penalty_path:
                Xinner_train, Xinner_test, yinner_train, yinner_test = CV(X,y, folds =folds)
                for j in range(folds//2):
                    reg.X = Xinner_train[j]
                    reg.y = yinner_train[j]

                    MSE, R2 = reg.svm(eps, penalty)
                    d = {"fit num":fnum,"MSE": MSE, "R2":R2, "unc. Tc": np.sqrt(MSE)*ymax, "epsilon":eps, "penalty": penalty, "data set": "train"}
                    d.update({"par%i"%k:reg.weights[k] for k in range(features)})
                    toi = toi.append(d, ignore_index=True)

                    MSE, R2 = reg.evaluation(Xinner_test[j], yinner_test[j])
                    d = {"fit num":fnum,"MSE": MSE, "R2":R2, "unc. Tc": np.sqrt(MSE)*ymax, "epsilon":eps, "penalty": penalty, "data set": "test"}
                    d.update({"par%i"%k:reg.weights[k] for k in range(features)})
                    toi =toi.append(d, ignore_index=True)

                    MSE, R2 = reg.evaluation(Xtest, ytest)
                    d = {"fit num":fnum,"MSE": MSE, "R2":R2, "unc. Tc": np.sqrt(MSE)*ymax, "epsilon":eps, "penalty": penalty, "data set": "eval"}
                    d.update({"par%i"%k:reg.weights[k] for k in range(features)})
                    toi = toi.append(d, ignore_index=True)
                    fnum +=1
    return toi, fnum

# Function that performs cross validation
def SVM_CV(X, y, ymax, epsilon_path, penalty_path, folds = 10):
    """
    Performs cross validation for SVM regression with hyperparameter tuning for epsilon and cost penalty (grid search)
    A nested CV of folds*folds/2 is performed for each element in epsilon
    kargs:
        folds:            number of cross validations
        Parameter paths:  list of hyper parameters to test
    returns tabel of information (toi) with all scores, parameters and hyperparameter
    """
    features = X.shape[1]
    reg = Regression()
    toi = pd.DataFrame(columns=["fit num","MSE","R2","unc. Tc", "epsilon","penalty", "data set",] + ["par%i"%i for i in range(features)])

    Xtrain, Xtest, ytrain, ytest = CV(X,y, folds = folds)
    fnum = 0
    for i in tqdm(range(folds)):
        toi, fnum = SVM_GridCV(Xtrain[i],ytrain[i], Xtest[i], ytest[i], epsilon_path,penalty_path, folds//2, toi, fnum, reg)
    return toi

def SVM_stats(toi, filepath, plot_par = False, features =81, skip_eval=False):
    """
    find optimal model by looking at toi, evaluate CV performance
    returns optimal parameters
    """
    f = open(filepath/"svm_stats2.txt",'w')

    #find best row based on test
    idx = toi[toi["data set"]=="test"]["MSE"].idxmin() # finding mininum MSE
    tabel = {"test": toi.iloc[idx]}
    tabel.update({ "train": toi.iloc[idx-1]})
    if not skip_eval:
        tabel.update({"eval": toi.iloc[idx +1]})
    f.write('Best model:\n')
    for data_set in ["train","test", "eval"]:
        if skip_eval & (data_set =='eval'):
            break
        f.write(data_set +': \n')

        for name in ["MSE", "R2", "unc. Tc", "epsilon","penalty"]:
            f.write(name + ': %.9f\n'%tabel[data_set][name])

        f.write("\n")

    # model variablity at best hyper parameter
    av = toi.groupby("data set").agg(["mean","std"])

    f.write('Average model:\n')
    for data_set in ["train","test", "eval"]:
        if skip_eval & (data_set =='eval'):
            break
        f.write(data_set +' CV results: \n')
        for name in ["MSE", "R2", "unc. Tc"]:

            f.write(name + ': %.9f +- %.9f\n'%(av.loc[data_set][name, "mean"],av.loc[data_set][name, "std"]))

        f.write("\n")
    f.close()

    inds3 = ["par%i"%i for i in range(features)]
    if plot_par:
        plt.figure(figsize =(10,10))
        inds = [("par%i"%i, 'mean') for i in range(features)]
        inds2 = [("par%i"%i, 'std') for i in range(features)]

        #95% confidence interval
        plt.errorbar(np.arange(features),av.loc["test"][inds], yerr=1.96*av.loc["test"][inds2], linestyle ='', marker='o', color ='tab:orange', label = r'95% conf. interv.')
        plt.plot(np.arange(features), tabel["test"][inds3], linestyle ='', marker ='x', color = 'tab:green', label ='Best param.')
        plt.legend(loc='best', fontsize = 28)
        plt.ylabel('par. value', fontsize =32)
        plt.xlabel("par. number", fontsize= 32)
        plt.xlim(-0.5, features+0.5)
        plt.tick_params(size =24, labelsize=26)
        plt.tight_layout()
        plt.savefig(filepath/'SVM_params2.pdf')

    return tabel["test"][inds3]

#  Plot the predicted versus the actual
def pred_vs_actual(X,y,ymax, weights, filepath):

    p = np.dot(X, weights)
    plt.figure(figsize=(10,10))
    plt.scatter(y*ymax, p*ymax)
    plt.plot([0,ymax],[0,ymax], linestyle ='--')
    plt.ylabel('act. T$_c$ in K', fontsize =32)
    plt.xlabel("pred. T$_c$ in K ", fontsize= 32)
    plt.xlim(0, ymax)
    plt.ylim(0, ymax)
    plt.tick_params(size =24, labelsize=26)
    plt.tight_layout()
    plt.savefig(filepath/'SVM_pred_vs_act2.pdf')

# Finding the best model

filep = Path("./Results/")
epsilon = [10**(-i) for i in range(1,6)]
penalty = [[10**i for i in range(1,6)] + [10**(-i) for i in range(7)] + [2*10**(-i) for i in range(1,7)] + [4*10**(-i) for i in range(1,7)] + [6*10**(-i) for i in range(1,7)]]
penalty = penalty[0]
print('Epsilon:{:}'.format(epsilon))
print('Penalty:{:}'.format(penalty))

skip = True
X,y,ymax = DataWorkflow()

#creat const. feature
ones = np.ones((X.shape[0],1))
X = np.hstack([ones,X])

features = X.shape[1]
reg = "SVM"
print('Regression: SVM')
print("Grid")
toi =SVM_CV(X,y,ymax,epsilon,penalty, folds =5)
toi.to_csv(filep/reg/'toi2.csv')
best_par = XGB_stats(toi, filep/reg, plot_par=True, features= features, skip_eval=skip)
pred_vs_actual(X[::50], y[::50], ymax , best_par, filep/reg)
